...
async def _adapt_metric_prompts_async(metrics: List[Any], evaluator_llm: Any) -> None:
    for metric in metrics:
        if hasattr(metric, "adapt_prompts") and hasattr(metric, "set_prompts"):
            try:
                adapted = await metric.adapt_prompts(language="korean", llm=evaluator_llm)
                if isinstance(adapted, dict):
                    metric.set_prompts(**adapted)
                    print(f"[INFO] Adapted prompts to Korean: {metric.__class__.__name__}")
                else:
                    print(f"[WARN] adapt_prompts 반환값이 dict가 아님: {metric.__class__.__name__}")
            except Exception as e:
                print(f"[WARN] metric prompt 한국어 변환 실패: {metric.__class__.__name__}: {e}")


def maybe_apply_korean_prompt_adaptation(metrics: List[Any], evaluator_llm: Any, enabled: bool) -> None:
    if not enabled:
        print("[INFO] 한국어 prompt adaptation을 적용하지 않습니다.")
        return

    print("[INFO] 한국어 prompt adaptation을 시도합니다...")
    try:
        asyncio.run(_adapt_metric_prompts_async(metrics, evaluator_llm))
    except Exception as e:
        print(f"[WARN] 한국어 prompt adaptation 실행 중 오류: {e}")

def build_metrics(
    evaluator_llm: Any,
    evaluator_embeddings: Any,
    use_korean_prompt_adaptation: bool = False,
) -> List[Any]:
    metrics = [
    ...
    maybe_apply_korean_prompt_adaptation(metrics, evaluator_llm, use_korean_prompt_adaptation)
    return metrics
...
def gather_runtime_config() -> Dict[str, Any]:
    ...
    limit = ask_int("파일럿 문항 수 제한 (0이면 전체)", 5)
    validate_only = ask_yes_no("모델 검증만 하고 종료할까요?", False)
    use_korean_prompt_adaptation = ask_yes_no("한국어 prompt adaptation을 적용할까요?", False)
    ...
    return {
        ...
        "use_korean_prompt_adaptation": use_korean_prompt_adaptation,
    }
...
def main() -> None:
    ...
    print(f"KOREAN PROMPT ADAPTATION : {cfg['use_korean_prompt_adaptation']}")
    ...
    metrics = build_metrics(
        ...
        use_korean_prompt_adaptation=cfg["use_korean_prompt_adaptation"],
    )
